...
class _DetailState extends State<Detail> {
  ...
  void initState() {
    super.initState();
    // Own a single diagnostic listener for the player's whole lifetime.
    _playbackSubscription = listenPlaybackEvents(_player);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: BlocListener<PokemonBloc, PokemonBlocState>(
        listenWhen: (previous, current) => current is PokemonBlocSuccess,
        listener: (context, state) {
          if (state is PokemonBlocSuccess) {
            // Side effects belong in a listener, never in build(): load the
            // audio source only when the cry actually changes.
            _setupAudioIfNeeded(state.pokemon.cry);
          }
        },
        child: PokemonBlocBuilder(
