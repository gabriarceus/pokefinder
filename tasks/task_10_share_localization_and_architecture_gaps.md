...
- [x] Add a share/copy action on the detail screen (app bar or header) that
      shares the canonical identifier (`/pokemon/:nameOrId`, e.g.
      `/pokemon/pikachu`, `/pokemon/25`).
- [x] Use clipboard copy as the baseline; add OS share sheet (`share_plus`
      or equivalent) only if it pulls no unjustified permissions.
- [x] Handle the incoming link: cold-start deep link / app link / universal
      link resolving to the existing canonical route with the not-found fallback
      for invalid values.
- [x] Localize all strings (EN + IT), add tooltip + screen-reader label, meet
      48×48 dp target.

### Acceptance criteria

- [x] Tapping share on a detail screen produces the canonical link for the
      currently displayed Pokémon/form.
- [x] Opening that link cold-resolves to the same Pokémon; invalid values hit
      the router error screen, never a cast crash.
- [x] No new permissions declared on Android or iOS for this feature.

### Tests

- [x] Unit: link build/parse round-trip (names, numeric IDs, invalid input).
- [x] Widget: share button renders label/tooltip, invokes share-or-copy;
...
- [x] Add an item-name translation database alongside the existing
      `lib/l10n/*_db.dart` files (abilities/moves/locations pattern), EN + IT,
      with a title-case fallback (never UPPER).
- [x] Audit and fix the same class of bug wherever it remains: raw domain
...
- [x] Add an automated translation-coverage test (abilities, moves, items,
...
- [x] Document which proper names intentionally stay canonical (e.g.
      Pokémon species names) vs. which must be localized.

### Acceptance criteria

- [x] Zero ALL-CAPS slug rendering in detail tabs; held items read like
      localized names with title-case fallback.
- [x] Coverage test exists and passes; adding a new slug without a
      translation (or documented fallback) fails CI.

### Tests

- [x] Unit: slug → display-name mapping (known EN/IT, unknown fallback,
      no UPPER output).
- [x] Widget: held-items section with unknown item renders fallback, never
      a raw slug in caps.
...
- [x] Decide ONE documented pattern and apply it: either introduce
...
- [x] Remove raw `getIt` calls from presentation: resolve through constructor
...
- [x] `ApiClient`/`LocalStorage` contracts are already generic/typed
      (`Future<T> get<T>`, `schemaVersion`) — no action; just don't regress to
      `dynamic`.

### Acceptance criteria

- [x] `MoveDetailCubit` path follows the same use-case-or-documented-exception
      rule as species/evolution/ability.
- [x] No unexplained `getIt<...>()` in `lib/src/1_presentation/`; the one
      allowed construction pattern is written down (one paragraph in README or
      `CLAUDE.md`).

### Tests

- [x] Unit: `GetMoveDetailUseCase` (or documented equivalent) success/failure
      mapping, including cancellation.
- [x] Widget: move-detail sheet builds through the new construction path and
...
- [x] Write a one-page policy (README section or `docs/logging_policy.md`):
...
- [x] Policy doc exists; release-mode log sample contains no raw query and
