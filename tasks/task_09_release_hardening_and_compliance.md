...
- **Presentation & Localization:**
  - `lib/src/1_presentation/pages/about/about_page.dart`
...
- [x] Replace `com.example.pokefinder` with an owned Android `applicationId` in `android/app/build.gradle`.
- [x] Configure secure Android release signing via keystore properties / environment variables outside Git tracking.
- [x] Remove `android:usesCleartextTraffic="true"` from `AndroidManifest.xml`.
- [x] Clean Android permissions: retain strictly `android.permission.INTERNET`.
- [x] Fix flavor app labels: link `android:label` to flavor-specific string resources (`@string/app_name`).
- [x] Add production Android adaptive icons and splash screen drawables.

### 3.2 iOS App Store Readiness (§4.7)
- [x] Replace `com.example.pokefinder` with an owned iOS Bundle Identifier in Xcode project configuration.
- [x] Clean up `ios/Runner/Info.plist`:
...
- [x] Configure Apple code signing and provision profile placeholders.
- [x] Add production iOS `AppIcon.appiconset` assets.

### 3.3 Flavor and Script Hardening (§4.7)
- [x] Connect Android build flavors (`dev`, `prod`) to Injectable environment configurations (`Environment.dev`, `Environment.prod`).
- [x] Fix VS Code `.vscode/tasks.json` task references and dependencies.
- [x] Update `scripts/post_build.dart` to exit with a non-zero exit code if Git tagging, release prerequisites, or pushes fail.
- [x] Block release tagging if the Git worktree is dirty.

### 3.4 Localization Polish & Number Formatting (§9.3)
- [x] Format height, weight, and stats using `intl` locale-aware formatters instead of hardcoded strings.
- [x] Polish Italian translation copy (e.g. fix `Cache svuota con successo` to `Cache svuotata con successo`).
- [x] Precompute and optimize sorted translation keys.

### 3.5 Logging and Privacy Policy (§9.5)
- [x] Ensure user-entered search queries are not logged in production release mode.
- [x] Truncate large HTTP response payloads in network logging interceptors.

### 3.6 In-App About & Attribution Screen (§9.6, §10)
- [x] Build an in-app "About PokéFinder" screen accessible from Settings:
...
- [x] Zero instances of `com.example` remain in Android or iOS project configurations.
- [x] Android and iOS builds declare only strictly required permissions; no microphone or background audio capabilities remain.
- [x] Android release build compiles with secure signing configuration and no cleartext traffic.
- [x] Release scripts abort immediately with an error if Git tagging fails or worktree is dirty.
- [x] The app displays a dedicated About screen with accurate PokeAPI credits and copyright disclaimers.
- [x] All mobile-applicable checklist items from [ROADMAP.md §10](file:///C:/Users/gabri/Documents/GitHub/pokefinder/ROADMAP.md#L1052-L1077) are satisfied.

---

## 5. Testing & Verification Plan

- [x] Inspect Android manifest and iOS `Info.plist` to verify minimal permissions.
- [x] Execute `scripts/post_build.dart` in a test environment to verify failure exit codes.
- [x] Verify localized string formatting across both English and Italian locales.
- [x] Build release APK / App Bundle and iOS archive locally:
