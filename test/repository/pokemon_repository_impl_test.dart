...
void main() {
  ...
  group('getPokemon type mapping', () {
    test('resolves both types from their resource URLs', () async {
      final pokemon = await mapPokemon(rawPokemonJson(types: [
        {
          'type': {'url': typeUrl(12)},
        },
        {
  ...
  group('getPokemon ability mapping', () {
    test('fills the ability slots in order and blanks the missing ones',
        () async {
      final pokemon = await mapPokemon(rawPokemonJson(abilities: [
        {
          'ability': {'name': 'overgrow'},
          'is_hidden': false,
          'slot': 1,
  ...
  group('getPokemon nested list flattening', () {
    test('expands one move entry per version group detail', () async {
      final pokemon = await mapPokemon(rawPokemonJson(moves: [
        {
    ...
    test('expands one held item per version detail', () async {
      final pokemon = await mapPokemon(rawPokemonJson(heldItems: [
        {
  ...
  group('getPokemon sprite mapping', () {
    test('leaves the official artwork unset when the payload omits it',
        () async {
      final pokemon = await mapPokemon(rawPokemonJson(sprites: {
        'front_default': 'front.png',
        'back_default': null,
        'front_shiny': null,
        'back_shiny': null,
  ...
  group('getPokemon failures', () {
    test('a datasource failure is propagated unchanged', () async {
      ...
      expect(result, left<PokemonFailure, Pokemon>(const UnauthorizedFailure()));
    });

    test('a malformed payload becomes an UnexpectedFailure instead of throwing',
        () async {
  ...
  group('getFormDetails', () {
    test('derives type sprites and official artwork from the form id',
        () async {
      when(() => dataSource.getFormDetails(any()))
          .thenAnswer((_) async => right(RawFormDetails.fromJson({
                'id': 10033,
                'name': 'venusaur-mega',
                'types': [
                  {
  ...
  group('getEncounters', () {
    test('exposes both the raw and the display-cased location name', () async {
      when(() => dataSource.getEncounters(any()))
  ...
  group('getMoveDetail', () {
    test('keeps the first flavor text per language', () async {
      when(() => dataSource.getMoveDetail(any()))
          .thenAnswer((_) async => right(RawMoveDetail.fromJson({
                'id': 33,
                'name': 'tackle',
                'accuracy': 100,
                'power': 40,
